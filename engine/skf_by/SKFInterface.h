...
#define SGD_AES256_ECB	0x00000801		//AES256算法ECB加密模式
#define SGD_AES256_CBC	0x00000802		//AES256算法CBC加密模式

#define	SGD_DES_ECB		0x00001001		//DES算法ECB加密模式
#define	SGD_DES_CBC		0x00001002		//DES算法CBC加密模式
...
//其他：	错误码。
//备注		权限要求：需要用户权限。
ULONG DEVAPI SKF_DeleteAllContainer(HAPPLICATION hApplication);
...
//ECC私钥解密
//用指定容器内的私钥对输入数据解密运算并输出结果
//参数		hContainer	[IN] 容器句柄
//			bSignFlag	[IN] FALSE--加密私钥，TRUE--签名私钥
//			pCipherText	[IN] 密文数据结构 
//			pbData		[OUT] 指向解密结果的缓冲区,如果值为NULL，用于取得解密后数据长度
//			pbDataLen	[IN, OUT] 调用时表示pbData的长度，返回解密数据的大小
//返回值	SAR_OK：	成功。
//其他：	错误码。
ULONG DEVAPI SKF_ECCDecrypt(HCONTAINER hContainer, BOOL bSignFlag, PECCCIPHERBLOB pCipherText, BYTE *pbData, ULONG *pulDataLen);


//RSA私钥解密
//使用指定容器内的RSA私钥对输入数据解密运算并输出结果
//参数		hContainer		[IN] 容器句柄
//			bSignFlag		[IN] TRUE表示签名公钥，FALSE表示加密公钥
//			pbInput			[IN] 指向待运算的原始数据缓冲区
//			ulInputLen		[IN] 待运算原始数据的长度，必须为公钥模长
//			pbOutput		[OUT] 指向RSA公钥运算结果缓冲区，如果该参数为NULL，则由pulOutputLen返回运算结果的实际长度
...
//返回值	SAR_OK：	成功。
//其他：	错误码。
ULONG DEVAPI SKF_RSADecrypt(HCONTAINER hContainer, BOOL bSignFlag, BYTE *pbInput, ULONG ulInputLen, BYTE *pbOutput, ULONG *pulOutputLen);


//导入RSA密钥对
//导入外部RSA密钥对到指定容器
//参数		hContainer	[IN] 容器句柄
//			bSignFlag	[IN] 要导入的密钥对类型 TRUE表示签名密钥对，FALSE表示加密密钥对
//			pBlob		[IN] RSA密钥对数据结构
//返回值	SAR_OK：	成功。
//其他：	错误码。
...
//返回值	SAR_OK：	成功。
//其他：	错误码。
ULONG DEVAPI SKF_ImportExtECCKeyPair(HCONTAINER hContainer, BYTE bSignFlag, ECCPUBLICKEYBLOB *sm2PubKey, ECCPRIVATEKEYBLOB * sm2PriKey);


//扩展接口：生成ECC密钥对
//函数原型	ULONG DEVAPI SKF_GenECCKeyPairExWT(HCONTAINER hContainer, ULONG ulAlgId， ECCPUBLICKEYBLOB *pBlob)
//功能描述	生成ECC签名密钥对并输出签名公钥。
//参数		hContainer	[IN] 密钥容器句柄。
//			bSignFlag	[IN] TRUE表示生成签名密钥对，FALSE表示生成加密密钥对
//			ulAlgId		[IN] 算法标识，只支持SGD_SM2_1算法。
//			pBlob		[OUT] 返回ECC公钥数据结构。
//返回值	SAR_OK：	成功。
//其他：	错误码。
//备注		权限要求：需要用户权限。
ULONG DEVAPI SKF_GenECCKeyPairExWT(HCONTAINER hContainer, BOOL bSignFlag, ULONG ulAlgId, ECCPUBLICKEYBLOB *pBlob);


//扩展接口：生成RSA密钥对
//函数原型	ULONG DEVAPI SKF_GenRSAKeyPairExWT(HCONTAINER hContainer, ULONG ulBitsLen, RSAPUBLICKEYBLOB *pBlob)
//功能描述	生成RSA签名密钥对并输出签名公钥。
//参数		hContainer	[IN] 容器句柄。
//			bSignFlag	[IN] TRUE表示生成签名密钥对，FALSE表示生成加密密钥对
//			ulBitsLen	[IN] 密钥模长。
...
//其他：	错误码。
//备注		权限要求：需要用户权限。
ULONG DEVAPI SKF_GenRSAKeyPairExWT(HCONTAINER hContainer, BOOL bSignFlag, ULONG ulBitsLen, RSAPUBLICKEYBLOB *pBlob);


//扩展接口：RSA签名
//函数原型	ULONG DEVAPI SKF_RSASignDataExWT(HCONTAINER hContainer, BOOL bSignFlag, BYTE *pbData, ULONG  ulDataLen, BYTE *pbSignature, ULONG *pulSignLen)
//功能描述	使用hContainer指定容器的签名私钥，对指定数据pbData进行数字签名。签名后的结果存放到pbSignature缓冲区，设置pulSignLen为签名的长度。
//参数		hContainer	[IN] 用来签名的私钥所在容器句柄。
//			bSignFlag	[IN] TRUE表示签名私钥运算，FALSE解密私钥运算
//			pbData		[IN] 被签名的数据。
//			ulDataLen	[IN] 签名数据长度，应不大于RSA密钥模长-11。
//			pbSignature	[OUT] 存放签名结果的缓冲区指针，如果值为NULL，用于取得签名结果长度。
//			pulSignLen	[IN，OUT] 输入时表示签名结果缓冲区大小，输出时表示签名结果长度。
...
//其他：	错误码。
//备注		权限要求：需要用户权限。
ULONG DEVAPI SKF_RSASignDataExWT(HCONTAINER hContainer, BOOL bSignFlag, BYTE *pbData, ULONG  ulDataLen, BYTE *pbSignature, ULONG *pulSignLen);


//扩展接口：删除容器内的密钥对或者证书
//函数原型	ULONG DEVAPI SKF_DeleteKeyPairOrCert(HCONTAINER hContainer,BOOL bSignFlag,BOOL bKeyPairFlag);
//功能描述	删除hContainer指定容器中的密钥对或者证书。
//参数		hContainer	[IN] 容器句柄。
//			bSignFlag	[IN] TRUE表示签名密钥对或者证书，FALSE解密密钥对或者证书
//			bKeyPairFlag[IN] TRUE表示删除密钥对，FALSE表示删除证书。
//返回值	SAR_OK：	成功。
//其他：	错误码。
//备注		权限要求：需要用户权限。
ULONG DEVAPI SKF_DeleteKeyPairOrCert(HCONTAINER hContainer, BOOL bSignFlag, BOOL bKeyPairFlag);


#ifdef VKF_DEF

//扩展接口：把指定的密钥的明文作为输入进行杂凑运算
//函数原型	ULONG DEVAPI SKF_DeleteKeyPairOrCert(HCONTAINER hContainer,BOOL bSignFlag,BOOL bKeyPairFlag)
//功能描述	相当于三步式数据杂凑运算第二步，把指定的密钥的明文作为输入进行杂凑运算。应用程序在调用SKF_DigestInit初始化后，可以重复调用SKF_DigestUpdate或VKF_DigestUpdateWithKey接口导入明文或密钥明文参与杂凑计算，最后调用SKF_DigestFinal获得计算结果。
//参数		hHash		[IN] 密码杂凑对象句柄。
//			hKey		[IN] 指定的会话密钥句柄。
//			ulKeyFormat	[IN] 密钥以何种格式作为输入明文参与杂凑算法：
//							0：直接用密钥明文二进制参与杂凑计算；
//							1：把密钥转换为BASE64格式后参与杂凑计算；
//							2：把密钥转换为十六进制小写字符串后参与杂凑计算；
//							3：把密钥转换为十六进制大写字符串后参与杂凑计算。
//返回值	SAR_OK：	成功。
//其他：	错误码。
ULONG DEVAPI VKF_DigestUpdateWithKey(HANDLE hHash, HANDLE hKey, ULONG ulKeyFormat);



//扩展接口：导入会话密钥并用密钥加密密钥解密，同时返回会话密钥句柄。
//函数原型	ULONG DEVAPI VKF_ImportKeyWithSessionKey(DEVHANDLE hDev, ULONG ulAlgId, HANDLE hSessionKeyHandle, BYTE *pbWrapedData, ULONG ulWrapedLen, HANDLE *phKey)
//功能描述	导入会话密钥并用密钥加密密钥解密，同时返回会话密钥句柄。
//参数		hDev				[IN] 设备句柄
//			ulAlgId				[IN] 会话密钥算法标识。
//			hSessionKeyHandle	[IN] 指定的事先已导入的会话密钥句柄。
//			pbWrapedData		[IN] 缓冲区指针，用于存放输入的密钥密文。
//			ulWrapedLen			[IN] 输入的密钥密文长度。
//			phKey				[OUT] 返回会话密钥句柄。
//返回值	SAR_OK：	成功。
//其他：	错误码。
ULONG DEVAPI VKF_ImportKeyWithSessionKey(DEVHANDLE hDev, ULONG ulAlgId, HANDLE hSessionKeyHandle, BYTE *pbWrapedData, ULONG ulWrapedLen, HANDLE *phKey);



//扩展接口：用ECC外部公钥加密输出会话密钥。
//函数原型	ULONG DEVAPI VKF_ECCExportSessionKeyByHandle(HANDLE hSessionKey, ECCPUBLICKEYBLOB *pPubKey, PECCCIPHERBLOB pData)
//功能描述	用ECC外部公钥加密输出会话密钥。
//参数		hSessionKey	[IN] Session句柄
//			pPubKey		[IN] 指定的会话密钥句柄。
//			pData		[IN] 密钥以何种格式作为输入明文参与杂凑算法：
//返回值	SAR_OK：	成功。
//其他：	错误码。
ULONG DEVAPI VKF_ECCExportSessionKeyByHandle(HANDLE hSessionKey, ECCPUBLICKEYBLOB *pPubKey, PECCCIPHERBLOB pData);

#endif // end: #ifdef VKF_DEF



#ifdef ZHUOWANG_DEF

//扩展接口：国密接口库中新增内部私钥解密接口
//函数原型  ULONG SKF_PrvKeyDecrypt(HCONTAINER hContainer, ULONG ulKeySpec, PECCCIPHERBLOB pCipherText, BYTE *pbData, ULONG *pdwDataLen)
//功能描述	用用指定容器内私钥解密。
//参数		hContainer	[IN] 容器句柄
//			ulKeySpec	[IN] 密钥类型：1：加密2：签名
//			pCipherBlob	[IN] 密文数据
//			pbData		[OUT] 返回明文数据
//			pulDataLen	[IN OUT] 输入输出参数，输入表示缓存区pbData大小，输出表示解密后的明文实际长度
//返回值	SAR_OK：	成功。
//			其他：	错误码。
ULONG DEVAPI SKF_PrvKeyDecrypt(HCONTAINER hContainer, ULONG ulKeySpec, PECCCIPHERBLOB pCipherBlob, BYTE *pbData, ULONG *pulDataLen);


//扩展接口：国密库增加 RSA 解密接口，使用指定容器下的指定类型的私钥(RSA)，对给出长度的密文进行解密，将解出的明文设置到指定位置并返回明文长度。
//函数原型  ULONG SKF_AsyPrvKeyDecrypt(HCONTAINER hContainer, ULONG ulKeySpec, BYTE *pbCiphertext, ULONG ulCiphertextLen, BYTE *pbData, ULONG *pulDataLen)
//功能描述	用用指定容器内私钥解密。
//参数		hContainer		[IN] 容器句柄
//			ulKeySpec		[IN] 密钥类型：1：加密2：签名
//			pbCiphertext	[IN] 密文数据
//			ulCiphertextLen	[IN] 密文数据长度
//			pbData			[OUT] 返回明文数据
//			pulDataLen		[IN OUT] 输入输出参数，输入表示缓存区pbData大小，输出表示解密后的明文实际长度
//返回值	SAR_OK：	成功。
//			其他：	错误码。
ULONG DEVAPI SKF_AsyPrvKeyDecrypt(HCONTAINER hContainer, ULONG ulKeySpec, BYTE *pbCiphertext, ULONG ulCiphertextLen, BYTE *pbData, ULONG *pulDataLen);

#endif // #ifdef ZHUOWANG_DEF

#endif // end: #ifndef BHA_DEF
